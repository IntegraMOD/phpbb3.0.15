...
class umil
{
    ...
    */
    public $auto_display_results = false;

    /**
    * Were any new permissions added (used in umil_frontend)?
    */
    public $permissions_added = false;
    ...
    */
     ...
     */
    public $stand_alone = false, $db = false)
    {
        // Setup $this->db
        if ($db !== false) {
        ...
        if (!$stand_alone) {
            ...
            } elseif (file_exists("{$phpbb_root_path}umil/language/" . basename((string) $config['default_lang']) . "/umil.$phpEx")) {
                $path = basename((string) $config['default_lang']);
            } elseif (file_exists("{$phpbb_root_path}umil/language/en/umil.$phpEx")) {
                $path = 'en';
            } else {
            ...
            if (is_array($info) && isset($info[0]) && isset($info[1]) && defined('DEBUG')) {
                if (version_compare(UMIL_VERSION, $info[0], '<')) {
                    ...
                    $user->lang['UPDATE_UMIL'] ??= 'This version of UMIL is outdated.<br /><br />Please download the latest UMIL (Unified MOD Install Library) from: <a href="%1$s">%1$s</a>';
    ...
    */
    public function get_output_text()
    {
        global $user;
    ...
    */
    public function run_actions($action, $versions, $version_config_name, $version_select = '')
    {
            ...
            foreach ($versions as $version => $version_actions) {
                ...
                foreach ($version_actions as $method => $params) {
                    if ($method == 'custom') {
                        ...
                        if (str_contains($method, 'update') || str_contains($method, 'table_insert') || str_contains($method, 'table_row_')) {
                            continue;
                        }
    ...
    */
    public function _call_custom_function($functions, $action, $version)
    {
    ...
    */
    public function multicall(mixed $function, mixed $params)
    {
    ...
    */
    public function cache_purge($type = '', $style_id = 0)
    {
                    ...
                    foreach ($cfg_data_imageset as $image_name => $value) {
                        if (str_contains((string) $value, '*')) {
                            if (str_ends_with((string) $value, '*')) {
                                ...
                                [$image_filename, $image_height, $image_width] = explode('*', (string) $value);
                            }
                        } else {
    ...
    */
    public function config_exists($config_name, $return_result = false)
    {
        global $config, $cache;
    ...
    * @return result
    */
    public function config_add($config_name, mixed $config_value = '', $is_dynamic = false)
    {
        // Multicall
    ...
    * @return result
    */
    public function config_update($config_name, mixed $config_value = '', $is_dynamic = false)
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $config_name)) {
            return;
    ...
    * @return result
    */
    public function config_remove($config_name)
    {
        global $cache, $config;
    ...
    */
    public function module_exists($class, $parent, $module)
    {
    ...
    */
    public function module_add($class, $parent = 0, $data = array(), $include_path = false)
    {
        ...
        if (!isset($data['module_langname'])) {
            ...
            $basename = str_replace(array('/', '\\'), '', (string) $basename);
    ...
    * @param string|bool $include_path If you would like to use a custom include path, specify that here
    */
    public function module_remove($class, $parent = 0, $module = '', $include_path = false)
    {
        global $cache, $user, $phpbb_root_path, $phpEx;
        ...
        if (is_array($module)) {
            ...
            // Automatic method
            $basename = str_replace(array('/', '\\'), '', (string) $module['module_basename']);
            $class = str_replace(array('/', '\\'), '', $class);
            $info_file = "$class/info/{$class}_$basename.$phpEx";
    ...
    */
    public function permission_exists($auth_option, $global = true)
    {
        ...
        if (str_starts_with($auth_option, 'f_')) {
            $global = false;
        }
    ...
    * @return result
    */
    public function permission_remove($auth_option, $global = true)
    {
        global $auth, $cache;
    ...
    */
    public function permission_role_update($old_role_name, $new_role_name = '')
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $role_name)) {
            return;
    ...
    */
    public function permission_role_remove($role_name)
    {
        global $auth;
    ...
    */
    public function table_add($table_name, $table_data = array())
    {
        global $dbms, $user;
    ...
    */
    public function table_column_add($table_name, $column_name = '', $column_data = array())
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    */
    public function table_column_update($table_name, $column_name = '', $column_data = array())
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    */
    public function table_column_remove($table_name, $column_name = '')
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    */
    public function table_index_add($table_name, $index_name = '', $column = array())
    {
        global $config;
    ...
    */
    public function table_index_remove($table_name, $index_name = '')
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    public function table_insert($table_name, $data = array())
    {
    ...
    */
    public function table_row_insert($table_name, $data = array())
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    */
    public function table_row_update($table_name, $data = array(), $new_data = array())
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    * WHERE user_id = 123 AND user_name = 'test user'
    */
    public function table_row_remove($table_name, $data = array())
    {
        // Multicall
        if ($this->multicall(__FUNCTION__, $table_name)) {
            return;
    ...
    */
    public function version_check($url, $path, $file, $timeout = 10, $port = 80)
    {
    ...
    */
    public function create_table_sql($table_name, $table_data, $dbms = '')
    {
        // To allow testing
        ...
        foreach ($table_data['COLUMNS'] as $column_name => $column_data) {
            // Get type
            if (str_contains((string) $column_data[0], ':')) {
                [$orig_column_type, $column_length] = explode(':', (string) $column_data[0]);
            ...
            if (is_array($column_data[1])) {
                $column_data[1] = $column_data[1][$dbms] ?? $column_data[1]['default'];
            }
            ...
            switch ($dbms) {
                ...
                case 'mysql_41':
                    ...
                    if (!is_null($column_data[1]) && !str_ends_with((string) $column_type, 'text') && !str_ends_with((string) $column_type, 'blob')) {
                        $sql .= (str_starts_with((string) $column_data[1], '0x')) ? "DEFAULT {$column_data[1]} " : "DEFAULT '{$column_data[1]}' ";
                    }
                    $sql .= 'NOT NULL';
                ...
                case 'firebird':
                    ...
                    if (preg_match('/^X?STEXT_UNI|VCHAR_(CI|UNI:?)/', (string) $column_data[0])) {
                        $sql .= ' COLLATE UNICODE';
                    }
                ...
                case 'mssqlnative':
                    ...
                    if (!is_null($column_data[1])) {
                        // For hexadecimal values do not use single quotes
                        if (str_starts_with((string) $column_data[1], '0x')) {
    ...
    */
    public function get_table_name(&$table_name)
    {
        ...
        */
        if (!preg_match('#^' . preg_quote((string) $table_prefix, '#') . '#', $table_name) || !in_array($table_name, $constants, true)) {
            if ((str_starts_with($table_name, (string) $table_prefix)) && (strlen($table_name) > strlen((string) $table_prefix))) {
                /**
                ...
                */
                ...
                $table_name = preg_replace('#^phpbb_#i', (string) $table_prefix, $table_name);
            }
        }
    }
}
