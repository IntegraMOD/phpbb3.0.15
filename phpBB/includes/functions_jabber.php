...
class jabber
{
	...
	*/
	function __construct($server, $port, $username, $password, $use_ssl, $verify_peer = null, $verify_peer_name = null, $allow_self_signed = null)
	{
		global $config;
...
/*		if (!$this->check_jid($this->username . '@' . $this->server))
	...
	*/
	function open_socket($server, $port, $use_ssl, $verify_peer, $verify_peer_name, $allow_self_signed)
	{
		...
		// PHP 8.0+ does not like "@" to suppress warnings
		$err = error_reporting();
		error_reporting(0);
		$this->connection = stream_socket_client($remote_socket, $errorno, $errorstr, $this->timeout, STREAM_CLIENT_CONNECT, $socket_context);
		error_reporting($err);
